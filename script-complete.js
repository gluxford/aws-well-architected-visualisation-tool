...
function displayWorkloadData(data) {
    ...
    try {
        ...
        document.getElementById('workload-name').textContent = data.workloadName || 'N/A';
        document.getElementById('report-date').textContent = new Date().toLocaleDateString();
        
        // Make sure we're displaying the correct data in each field
        document.getElementById('reviewer-name').textContent = data.ownerName || 'N/A';
        document.getElementById('account-id').textContent = data.accountIds && data.accountIds.length > 0 ? data.accountIds.join(', ') : 'N/A';
        document.getElementById('region-display').textContent = data.regions && data.regions.length > 0 ? data.regions.join(', ') : 'N/A';
    ...
    document.getElementById('compliant-risk').textContent = data.riskCounts.compliant;  // Changed from 'low-risk' to 'compliant-risk'
    ...
    createRiskChart(data.riskCounts.high, data.riskCounts.medium, data.riskCounts.compliant);  // Changed to use compliant instead of low and none
